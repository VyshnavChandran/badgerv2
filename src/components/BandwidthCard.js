...
const BandwidthCard = ({ publishingHouse, publishers, bandwidthDataMap, fetchBandwidthData, expanded, onExpandToggle }) => {
  const [selectedPublisher, setSelectedPublisher] = useState(0);

  const publisherName = publishers[selectedPublisher]?.name;
  const publisherData = bandwidthDataMap[publisherName]?.data || [];
  const loading = bandwidthDataMap[publisherName]?.loading || false;

  useEffect(() => {
    if (publisherName && !bandwidthDataMap[publisherName]) {
  ...
  const handleRefresh = () => {
    fetchBandwidthData(publisherName, true);
  };
    ...
    <Card sx={{ width: '100%', maxWidth: 1200, mx: 'auto', mb: 4, boxSizing: 'border-box', borderBottom: '2px solid #000' }}>
      <CardContent>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', mt: 2 }}>
          <Box sx={{ flexGrow: 1, width: '100%', minWidth: 0, height: 300 }}>
            {loading ? (
              ...
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={chartData} margin={{ top: 20, right: 30, left: 0, bottom: 0 }}>
                  ...
                  <XAxis dataKey="month" />
                  <YAxis width={70} fontSize={10} tickFormatter={compactNumber} />
          ...
          <Table size="small" sx={{ minWidth: 700 }}>
            <TableHead>
              <TableRow>
                ...
                {metrics.map(metric => (
                  <TableCell key={metric.key} align="right">{metric.label}{metric.isGB ? ' (GB)' : ''}</TableCell>
                ))}
              </TableRow>
            </TableHead>
            <TableBody>
