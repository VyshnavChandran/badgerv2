...
import { Card, CardContent, Typography, Box, Grid, CircularProgress, ToggleButton, ToggleButtonGroup, Dialog, DialogContent } from '@mui/material';
...
const AllPublishers = ({ publishingHouses, bandwidthData, processedGraphData, loading }) => {
  ...
  const ChartComponent = useMemo(() => ({ data, onClick }) => (
    <ResponsiveContainer width="100%" height="100%">
      <LineChart data={data} margin={{ top: 10, right: 10, left: 0, bottom: 0 }} onClick={onClick}>
  ...
  const handleCardClick = useCallback((publisher) => {
    ...
    <Layout search={search} setSearch={setSearch}>
      ...
      >
        <DialogContent>
          {selectedPublisher && (
            <BandwidthCard
              publishingHouse={selectedPublisher['Publishing House__name'] || 'Uncategorized'}
